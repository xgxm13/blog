...
* 区别请移步 [Css 预处理语言Sass、Less、Stylus区别](https://blog.mengcodes.com/Web/Css/2023/09/04/1//)
...
* 引用类型：Object: Array – 数组对象、RegExp – 正则对象、Date – 日期对象、Math – 数学函数、Function – 函数对象等常见类型
* 其中基础数据类型存储在栈内存，被引用或拷贝时，会创建一个完全相等的变量。引用类型存储在堆内存，存储的是地址，多个引用指向同一个地址。假如声明两个引用类型同时指向了一个地址的时候，修改其中一个那么另外一个也会改变。
:::

* 数据类型转换
    强制类型转换: Number()、parseInt()、parseFloat()、Boolean()等
:::info
* Number() 方法的强制转换规则：
...
:::
* 区别请移步 [Js 检测数据类型的方法和区别](https://blog.mengcodes.com/Web/Js/2023/09/04/2/) 
...
使用场景：防抖，节流，函数嵌套函数避免全局污染，自执行的时候
:::
...
方式：addEventListener('click',函数名，true/false) 默认是false（事件冒泡），true（事件捕获）
